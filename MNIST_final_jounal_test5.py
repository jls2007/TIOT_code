...
class NetA(nn.Module):
    def __init__(self):
        ...
        self.norm=nn.BatchNorm2d(800,affine=False,momentum=0.1)
...
def train1(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output1 = modelA(data)
        #print(output1.size()[1])
        temnoise=np.random.laplace(loc,scale, (output1.size()[0],output1.size()[1]))
        output1=output1+Variable(torch.from_numpy(temnoise).type(torch.FloatTensor).cuda())

        optimizer2.zero_grad()
        
        output2=modelB(output1.detach())
